...
The YAML profile is optional. Without one, every table that has a generator is emitted using the default `contoso.com` tenant fixture. With one, you can select a subset of tables and/or override fixtures so the stream looks like it came from *your* tenant. Every override replaces the matching default in full — collections are never merged.
